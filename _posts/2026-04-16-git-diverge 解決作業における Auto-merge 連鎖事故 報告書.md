...
#### GitHub UI とローカル CLI で commit の落ち先が異なる

冲突解決 commit がどのブランチに落ちるかは、操作環境によって異なる。

| 操作環境 | commit の落ち先 | 根拠 |
|---|---|---|
| **GitHub UI** | **head branch**（from 側） | GitHub が head branch に commit を作成する |
| **ローカル CLI** | **現在 checkout しているブランチ** | git merge は常に現在の HEAD に commit を落とす |

```
PR2932: prep-trivy(head) → conflict-resolve(base)

GitHub UI で冲突解決:
  commit S1 → prep-trivy（head）に落ちる
  → C1 が S1 の ancestor になる
  → PR2926 の fast-forward が成立 → 事故発生 ❌

ローカル CLI で同じ冲突解決をした場合:
  git checkout conflict-resolve   ← base を checkout
  git merge prep-trivy
  → commit S1 → conflict-resolve（base）に落ちる
  → prep-trivy は動かない
  → C1 は S1 の ancestor にならない
  → PR2926 の fast-forward は成立しない → 事故回避 ✅
```

**GitHub UI を使って冲突を解決したことが、fast-forward を引き起こした直接的なトリガーであった。**
...
| P5 | GitHub UI と ローカル CLI の挙動差の未把握 | GitHub UI で冲突解決すると commit が head branch に落ちる。ローカル CLI では checkout 中のブランチに落ちる。この差を認識していなければ、どちらで操作するかの判断ができない |
...
**代替手段：ローカル CLI で merge（中間ブランチ不要）**

PR を使わずにローカルで merge することで、commit の落ち先を自分でコントロールできる。
main の保護ルールにも触れない。

```bash
git checkout feature/B        ← 冲突解決 commit を落としたいブランチを checkout
git merge origin/main         # = git fetch + git merge origin/main
# → 冲突解決 commit は feature/B に落ちる
git push origin feature/B
```

```
GitHub UI:     commit → head branch（from 側）に落ちる
ローカル CLI:  commit → 現在 checkout しているブランチに落ちる ← 落ち先を制御できる
```

ローカル merge は**個人ブランチでは常に有効**。
公共ブランチの場合は code freeze かつ全員周知の条件を満たすこと（Case 1 と同条件）。
...
**③ 冲突解決は GitHub UI ではなくローカル CLI で行う**

GitHub UI と ローカル CLI では冲突解決 commit の落ち先が異なる。
逆方向 PR が存在する状況では、**ローカル CLI で base branch を checkout して merge する**ことで
fast-forward の成立を防ぐことができる。

```bash
# PR: feature/B(head) → conflict-resolve(base) の冲突を解決する場合

# ❌ GitHub UI: commit が head(feature/B) に落ちる → fast-forward 成立リスク

# ✅ ローカル CLI: base を checkout して merge
git checkout conflict-resolve   ← base branch
git merge feature/B
# → commit は conflict-resolve に落ちる
# → feature/B は動かない → fast-forward 不成立
```
